		...
		uint32_t name, const char *interface, uint32_t version)
{
	if (strcmp(interface, wl_output_interface.name) == 0) {
		struct output *tracked = NULL;
		if (watch_outputs) {
			tracked = calloc(1, sizeof(*tracked));
			if (!tracked) die("out of memory");
			tracked->name = name;
			tracked->next = outputs;
			outputs = tracked;
		}
		struct wl_output *o = wl_registry_bind(
				wl_registry, name, &wl_output_interface,
				WL_OUTPUT_NAME_SINCE_VERSION);
		wl_output_add_listener(o, &output_listener, tracked);
