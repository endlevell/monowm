...
/* enums */
enum { CurNormal, CurPressed, CurMove, CurResize, CurTileDrag, CurCanvasPan };
...
struct Monitor {
  double zoom;
  uint32_t canvas_tags;      /* bitmask: which tags are in canvas mode */
  double canvas_x, canvas_y; /* current pan offset for the active tag */
  double pan_start_cx, pan_start_cy;   /* cursor position when pan began */
  double pan_start_ox, pan_start_oy;   /* canvas_x/y when pan began */
	struct wl_list link;
...
void canvas_layout(Monitor *m);
void canvaspan(const Arg *arg);
...
applybounds(Client *c, struct wlr_box *bbox)
{
	...
	if (c->mon && (c->mon->canvas_tags & c->tags))
		return; /* infinite canvas: never clamp position to the monitor */
...
void dispatch_action(const char *action, const char (*args)[CFG_MAX_STRLEN], int nargs);
...
{
	if (!session) {
		fprintf(stderr, "mono: chvt: no session (not running on DRM)\n");
		return;
	}
...
dispatch_action(const char *action, const char (*args)[CFG_MAX_STRLEN], int nargs)
{
  ...
  if (!strcmp(action, "viewshift")) {
	  if (a0) arg.i = atoi(a0);
	  viewshift(&arg);
	  return;
  }
  if (!strcmp(action, "togglecanvas")) {
	togglecanvas(&arg);
	return;
  }
  if (!strcmp(action, "canvaspan")) {
	  canvaspan(&arg);
	  return;
  }
  if (!strcmp(action, "tagshift")) {
	  if (a0) arg.i = atoi(a0);
	  tagshift(&arg);
	  return;
  }
  if (!strcmp(action, "movecenter")) {
	  if (a0) arg.i = atoi(a0);
	  movecenter(&arg);
	  return;
  }
	...
	fprintf(stderr, "mono: unknown action '%s'\n", action);
}
...
keybinding(uint32_t mods, xkb_keysym_t sym)
{
	for (int i = 0; i < cfg.nkeybinds; i++) {
		...
		if (CLEANMASK(mods) == CLEANMASK(k->mods)
			...
			if (k->lua_ref != LUA_NOREF) {
				lua_rawgeti(g_L, LUA_REGISTRYINDEX, k->lua_ref);
				if (lua_pcall(g_L, 0, 0, 0) != LUA_OK) {
					fprintf(stderr, "mono: lua keybind error: %s\n",
					        lua_tostring(g_L, -1));
					lua_pop(g_L, 1);
				}
			} else {
...
maximizenotify(struct wl_listener *listener, void *data)
{
	 ...
	 * client-side decorations. mono doesn't support maximization, but
		...
		double dx_unaccel, double dy_unaccel)
{
	...
	if (cursor_mode == CurMove) {
		...
		if (grabc->mon && (grabc->mon->canvas_tags & grabc->mon->tagset[grabc->mon->seltags])) {
			grabc->world_width = grabc->geom.width / grabc->mon->zoom;
			grabc->world_height = grabc->geom.height / grabc->mon->zoom;
		}
		return;
...
void
canvaspan(const Arg *arg)
{
	if (cursor_mode != CurNormal && cursor_mode != CurPressed)
		return;
	if (!selmon || !(selmon->canvas_tags & selmon->tagset[selmon->seltags]))
		return;
	cursor_mode = CurCanvasPan;
	selmon->pan_start_cx = cursor->x;
	selmon->pan_start_cy = cursor->y;
	selmon->pan_start_ox = selmon->canvas_x;
	selmon->pan_start_oy = selmon->canvas_y;
...
run(char *startup_cmd)
{
	 ...
	 * causing mono to freeze when a user neither closes stdin
...
dwindle_insert(DwindleNode **root, Client *new_c, Client *focused, DwindleHint *hint)
{
	...
	if (split_top) {
		int mid_y = opening_on->box.y + opening_on->box.height / 2;
		new_first = (cursor->y < mid_y);
	} else {
		int mid_x = opening_on->box.x + opening_on->box.width / 2;
		new_first = (cursor->x < mid_x);
	}

	if (hint) {
		hint->old_box = opening_on->box;
		hint->edge = split_top
			? (new_first ? EDGE_TOP    : EDGE_BOTTOM)
			: (new_first ? EDGE_LEFT   : EDGE_RIGHT);
	}
	...
	new_parent->box           = opening_on->box;
	new_parent->parent        = opening_on->parent;
	new_parent->split_top     = split_top;
	new_parent->children[0]   = new_first ? new_leaf : opening_on;
	new_parent->children[1]   = new_first ? opening_on : new_leaf;
...
dwindle(Monitor *m)
{
...

	{
		...
		for (int i = 0; i < sc; i++)
			dwindle_remove(root, stale[i], NULL);
	}
	...
	wl_list_for_each(c, &clients, link) {
		...
		if (!dwindle_find_leaf(*root, c)) {
			dwindle_insert(root, c, focused, NULL);
			focused = c;
		}
	}
...
viewshift(const Arg *arg)
{
	...
	uint32_t cur = selmon->tagset[selmon->seltags] & TAGMASK;
	int idx = 0;
	if (cur) {
		while (!(cur & 1u)) { cur >>= 1; idx++; }
	}

	idx = (idx + arg->i) % TAGCOUNT;
	if (idx < 0)
		idx += TAGCOUNT;
...
movecenter(const Arg *arg)
{
	...
	if (!sel || !sel->isfloating)
		return;
	sel->geom.x = (selmon->w.width - sel->geom.width) / 2 + selmon->m.x;
	sel->geom.y = (selmon->w.height - sel->geom.height) / 2 + selmon->m.y;
	resize(sel, sel->geom, 0);
}
...
xwaylandready(struct wl_listener *listener, void *data)
{
	...
	/* Set the default XWayland cursor to match the rest of mono. */
